...
server_socket.bind((HOST, PORT)) #綁定IP 和 PORT
server_socket.listen(5) #最大連接數量，超過則拒絕連接
print("server:{} port:{} start".format(HOST, PORT)) #顯示伺服器IP 跟 PORT
...
while True:
    msg = client.recv(128).decode("utf8") #接受客戶端訊息，100為接收訊息的最大長度，utf8為解碼方式
    print("Receive Massage:" + msg)
    reply = "" #建立伺服器回應字串
